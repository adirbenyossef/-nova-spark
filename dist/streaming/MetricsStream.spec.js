...
const MetricsStream_1 = require("./MetricsStream");
describe('MetricsStream', () => {
    ...
    beforeEach(() => {
        config = new Config_1.Config({ metricsBufferSize: 3 });
        stream = new MetricsStream_1.MetricsStream(config);
    });
    /**
    ...
    it('should respect buffer size configuration', (done) => {
        const stream = new MetricsStream_1.MetricsStream(new Config_1.Config({ metricsBufferSize: 3 }));
        const metrics = Array.from({ length: 5 }, (_, i) => createMetric(`test.metric${i}`, i * 100));
        let emitCount = 0;
        stream.on('data', (emittedMetrics) => {
            emitCount++;
            if (emitCount === 1) {
    ...
    */
    it('should emit immutable metric copies', (done) => {
        const stream = new MetricsStream_1.MetricsStream(new Config_1.Config());
